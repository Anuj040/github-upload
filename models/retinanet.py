...
from FCOS.utils.anchors import AnchorParameters
from FCOS import layers
import layers
import initializers
def default_shared_model(
    ...
    return KM.Model(inputs=inputs, outputs=outputs, name=name)
...
def default_regression_model(num_values=4, pyramid_feature_size=256, regression_feature_size=256,
    ...
    Returns
        A custom model that predicts regression values for each anchor.
    """
    ...
    outputs = KL.Conv2D(num_values, name='pyramid_regression', **options)(outputs)
    # (b, num_anchors_this_feature_map, num_values)
    outputs = KL.Reshape((-1, num_values), name='pyramid_regression_reshape')(outputs)
    # added for fcos
    outputs = KL.Lambda(lambda x: K.exp(x))(outputs)
    return KM.Model(inputs=inputs, outputs=outputs, name=name)
...
def default_classification_model(
    ...
    outputs = KL.Reshape((-1, num_classes), name='pyramid_classification_reshape')(outputs)
    outputs = KL.Activation('sigmoid', name='pyramid_classification_sigmoid')(outputs)
...
def default_centerness_model(
    ...
    # reshape output and apply sigmoid
    outputs = KL.Reshape((-1, 1), name='pyramid_centerness_reshape')(outputs)
    outputs = KL.Activation('sigmoid', name='pyramid_centerness_sigmoid')(outputs)
...
def __build_model_pyramid(model_name, model, features):
    ...
    """
    return KL.Concatenate(axis=1, name=model_name)([model(f) for f in features])
...
def retinanet(
    ...
    return KM.Model(inputs=inputs, outputs=pyramids, name=name)
...
def retinanet_bbox(
    ...
    # construct the model
    return KM.Model(inputs=model.inputs, outputs=detections, name=name)
